...
function buildCompanyClaimRow(
    ...
    return [
        ...
        'product_code' => sprintf('CCS%06dP%03d', $serviceOrderId, $sourceProductId),
        'product_name' => trim((string) ($claimDetails['company_claim_product_name'] ?? '')) ?: ($sourceProductName !== '' ? $sourceProductName : ('Claim Product ' . $sourceProductId)),
        'source_product_name' => $sourceProductName,
        'serial_number' => trim((string) ($claimDetails['company_claim_serial_number'] ?? '')),
        'is_spare_product' => 0,
        'brand' => trim((string) ($claimDetails['company_claim_brand'] ?? '')),
        'model' => trim((string) ($claimDetails['company_claim_model'] ?? '')),
        'category' => 'service',
        'claim_type' => 'company_claim',
        'company_claim_product_name' => trim((string) ($claimDetails['company_claim_product_name'] ?? '')),
        'company_claim_brand' => trim((string) ($claimDetails['company_claim_brand'] ?? '')),
        'company_claim_model' => trim((string) ($claimDetails['company_claim_model'] ?? '')),
        'company_claim_serial_number' => trim((string) ($claimDetails['company_claim_serial_number'] ?? '')),
...
function loadCompanyClaimRows(PDO $conn): array {
    $query = "SELECT so.id,
                     so.order_code,
                     so.client_id,
    ...
    while ($order = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        foreach ($claimTypeMap as $productId => $mappedClaimType) {
            ...
            if (normalizeClaimTypeValue($mappedClaimType) !== 'company_claim') {
                continue;
            }
            ...
            $companyClaimRows[] = buildCompanyClaimRow($order, $sourceProductId, $claimDetails, $sourceProductNameMap);
        }
    }

    return $companyClaimRows;
}

function matchesCompanyClaimSearch(array $product, string $search): bool {
    if ($search === '') {
        return true;
    }
    ...
    $searchableValues = [
        ...
        $product['brand'] ?? '',
        $product['model'] ?? '',
        $product['company_claim_product_name'] ?? '',
...
try {
    ...
    $companyClaimRows = loadCompanyClaimRows($conn);

    if (isset($_GET['id']) && $_GET['id'] !== '') {
        $requestedId = (int) $_GET['id'];
        $matchedProduct = null;

        foreach ($companyClaimRows as $row) {
        ...
        if ($matchedProduct === null) {
            ...
            echo json_encode(["success" => false, "message" => "Company claim service not found"]);
            exit();
        }
    ...
    $filteredProducts = array_values(array_filter($companyClaimRows, static function (array $product) use ($search, $hasStatusFilter, $statusFilter, $startDate, $endDate): bool {
        ...
        return matchesCompanyClaimSearch($product, $search);
    }));
