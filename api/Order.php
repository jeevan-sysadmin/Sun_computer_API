...
function normalizeClaimTypeMap($value) {
...
function normalizeClaimDetailRow($value) {
    $fields = [
...
function normalizeClaimDetailsMap($value) {
    if (!is_array($value)) {
        return [];
    }
    ...
    foreach ($value as $productId => $row) {
        ...
        $normalized[$id] = normalizeClaimDetailRow($row);
    }
...
function hydrateProductStatusMapForProducts($product_ids, $statusMap, $fallbackStatus = 'pending') {
    $product_ids = array_values(array_unique(array_filter(array_map('intval', $product_ids), function ($id) {
        return $id > 0;
    })));
    ...
    foreach ($product_ids as $product_id) {
        if (!isset($statusMap[$product_id]) || trim((string)$statusMap[$product_id]) === '') {
            $statusMap[$product_id] = $fallback;
            continue;
        }

        $normalizedStatus = strtolower(trim((string)$statusMap[$product_id]));
        $statusMap[$product_id] = in_array($normalizedStatus, $allowed, true) ? $normalizedStatus : $fallback;
    }

    return $statusMap;
}

function hydrateClaimTypeMapForProducts($db, $product_ids, $claimTypeMap, $fallbackClaimType = null) {
    $product_ids = array_values(array_unique(array_filter(array_map('intval', $product_ids), function ($id) {
        return $id > 0;
    })));
    ...
    $missing_ids = array_values(array_filter($product_ids, function ($product_id) use ($claimTypeMap) {
        return !isset($claimTypeMap[$product_id]) || trim((string)$claimTypeMap[$product_id]) === '';
    }));

    if (!empty($missing_ids)) {
        $placeholders = implode(',', array_fill(0, count($missing_ids), '?'));
        $stmt = $db->prepare("SELECT id, claim_type FROM products WHERE id IN ($placeholders)");
        $stmt->execute($missing_ids);

        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $product_id = (int)$row['id'];
            $claimType = strtolower(trim((string)($row['claim_type'] ?? 'none')));
            $claimTypeMap[$product_id] = in_array($claimType, $allowed, true) ? $claimType : $fallback;
        }
    }

    foreach ($product_ids as $product_id) {
        if (!isset($claimTypeMap[$product_id]) || trim((string)$claimTypeMap[$product_id]) === '') {
            $claimTypeMap[$product_id] = $fallback;
        }
    }
...
function syncSelectedProductClaims($db, $product_ids, $claimTypeMap) {
    $product_ids = array_values(array_unique(array_filter(array_map('intval', $product_ids), function ($id) {
        return $id > 0;
    })));
    ...
    $productColumns = fetchTableColumns($db, 'products');
    ...
    foreach ($product_ids as $product_id) {
        ...
        $claimType = isset($claimTypeMap[$product_id]) ? $claimTypeMap[$product_id] : 'none';
...
function syncOrderClaimData($db, $order_id, $claimTypeMap, $claimDetailsMap, $productStatusMap = []) {
    ...
    $primaryClaimDetails = $primaryProductId && isset($claimDetailsMap[$primaryProductId])
        ? normalizeClaimDetailRow($claimDetailsMap[$primaryProductId])
        : normalizeClaimDetailRow([]);
...
function ensureDeliveredOrderDelivery($db, $order_id) {
    $order_id = (int)$order_id;
    if ($order_id <= 0) {
        return;
    }

    $deliveryColumns = fetchTableColumns($db, 'deliveries');
    ...
    $orderStmt->bindValue(':id', $order_id, PDO::PARAM_INT);
    $orderStmt->execute();
    ...
    $deliveryCode = 'DEL' . str_pad((string)$order_id, 6, '0', STR_PAD_LEFT);
    ...
    $notes = 'Auto-created delivery record for order ' . ($order['order_code'] ?? ('#' . $order_id)) . ' - Product delivered';

    $existingStmt = $db->prepare("SELECT id FROM deliveries WHERE order_id = :order_id ORDER BY id DESC LIMIT 1");
    $existingStmt->bindValue(':order_id', $order_id, PDO::PARAM_INT);
    ...
    $insertQuery = "INSERT INTO deliveries (
                        order_id,
                        delivery_code,
                        delivery_type,
                        address,
...
try {
    switch ($method) {
        ...
        case 'POST':
            ...
            $selected_product_statuses = normalizeProductStatusMap($input['selected_product_statuses'] ?? []);
            $selected_product_claim_types = normalizeClaimTypeMap($input['selected_product_claim_types'] ?? []);
            $selected_product_claim_details = normalizeClaimDetailsMap($input['selected_product_claim_details'] ?? []);
            $selected_product_statuses = hydrateProductStatusMapForProducts(
                $product_ids,
                $selected_product_statuses,
                $input['status'] ?? 'pending'
            );
            $selected_product_claim_types = hydrateClaimTypeMapForProducts(
                $db,
                $product_ids,
            ...
            try {
                ...
                syncSelectedProductClaims($db, $product_ids, $selected_product_claim_types);
                syncOrderClaimData($db, $order_id, $selected_product_claim_types, $selected_product_claim_details, $selected_product_statuses);
                ensureDeliveredOrderDelivery($db, $order_id);
        ...
        case 'PUT':
            ...
            $orderColumns = fetchTableColumns($db, 'service_orders');
            $hasOrderColumns = !empty($orderColumns);
            $existingOrderStmt = $db->prepare("SELECT product_id, product_ids FROM service_orders WHERE id = :id");
            $existingOrderStmt->bindValue(':id', $id, PDO::PARAM_INT);
            $existingOrderStmt->execute();
            $existing_order = $existingOrderStmt->fetch(PDO::FETCH_ASSOC);
            ...
            $claim_source_product_ids = normalizeIdList($existing_order['product_ids'] ?? ($existing_order['product_id'] ?? null));
            ...
            if (array_key_exists('product_ids', $input) || array_key_exists('product_id', $input)) {
                ...
                $claim_source_product_ids = $product_ids;
            }
            ...
            try {
                ...
                if (!is_null($update_product_ids)) {
                    ...
                    syncSelectedProductClaims($db, $update_product_ids, $selected_product_claim_types);
                }
                ...
                if (!is_null($update_product_ids) || array_key_exists('selected_product_statuses', $input) || array_key_exists('selected_product_claim_types', $input) || array_key_exists('selected_product_claim_details', $input)) {
                    $claimTargetIds = !is_null($update_product_ids)
                        ? $update_product_ids
                        : normalizeIdList($existing_order['product_ids'] ?? ($existing_order['product_id'] ?? null));
                    syncOrderClaimData($db, (int)$id, $selected_product_claim_types, $selected_product_claim_details, $selected_product_statuses);
                    syncSelectedProductClaims($db, $claimTargetIds, $selected_product_claim_types);
                }

                ensureDeliveredOrderDelivery($db, (int)$id);
