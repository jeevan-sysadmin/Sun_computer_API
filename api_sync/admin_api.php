...
class AdminAPI {
    ...
    private function syncSelectedProductClaims(array $productIds, array $claimTypeMap): void {
        ...
        $productColumns = fetchTableColumns($this->conn, 'products');
    ...
    private function syncOrderClaimData(int $orderId, array $claimTypeMap, array $claimDetailsMap): void {
        $orderColumns = fetchTableColumns($this->conn, 'service_orders');
        ...
        try {
            ...
            try {
                ...
                if ($orderStmt->execute()) {
                    ...
                    $this->syncOrderClaimData((int)$order_id, $selected_product_claim_types, $selected_product_claim_details);
        ...
        try {
            ...
            try {
                ...
                if ($stmt->execute()) {
                    ...
                    if ($productIdsProvided || array_key_exists('selected_product_claim_types', $data) || array_key_exists('selected_product_claim_details', $data)) {
                        $claimTargetIds = !is_null($new_product_ids)
                            ? $new_product_ids
                            : $this->normalizeIdList($existingOrder['product_ids'] ?? ($existingOrder['product_id'] ?? null));
                        $this->syncOrderClaimData((int)$data['id'], $selected_product_claim_types, $selected_product_claim_details);
